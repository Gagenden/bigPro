<template>
  <div id="changePass">
    ...
    <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="100px" class="demo-ruleForm">
      <el-form-item label="学生账号：" prop="num">
        <el-select v-model="ruleForm2.num" placeholder="请选择学生账号">
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          ...
          if (valid) {
            //发起修改密码的post请求
            this.axios.post(this.HOST + '/admin/changePassS',{
              studentId: this.ruleForm2.num,
              newpass: this.ruleForm2.newPass
            }).then(result => {
              console.log(result);
              if(result.data.changePasswordS == true){
                alert('修改密码成功');
                // 修改密码成功之后跳转
      ...
      bindStudentId(){
        this.axios.get(this.HOST + '/admin/changePassS').then(res => {
        //学生账号
        for(var i=0; i<res.data.length; i++){
          //因为账号是唯一的，所以不需要去重
          this.options.push(res.data[i].studentId);
        }
      });
      }
    },
