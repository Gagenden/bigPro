  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          this.axios
            .get(this.HOST + "/student/changePasswordByS", {
              params: {
            ...
            .then(result => {
              ...
              if (result.data.changePasswordByS == true) {
                alert("修改密码成功！");
                this.$router.push({ path: "/student/stuInfo" });
              } else {
                alert("修改密码失败！");
              }
            });
    ...
    init() {
      this.axios
        .post(this.HOST + "/student/changePasswordByS", {
          studentId: sessionStorage.getItem("studentName")
        })
        .then(result => {
          console.log(result.data);
