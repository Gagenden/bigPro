  ...
  <div id="changePass">
    ...
    <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="100px" class="demo-ruleForm">
      <el-form-item label="原密码：" prop="oldPass">
        <el-input type="password" v-model="ruleForm2.oldPass" autocomplete="off" class="inputSize" placeholder="请输入原密码" show-password></el-input>
      </el-form-item>
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          //校验通过
          if (valid) {  //发起修改密码的请求（账号  新密码）
            this.axios.get(this.HOST + '/teacehr/changePasswordByT',{
              params: {
              ...
              if(result.data.changePasswordByT == true){
                alert('修改密码成功！');
                this.$router.push({path: '/teacher/teaInfo'});
              }else{  
                alert('修改密码失败！');
              }
            })
      ...
      // 从数据库中获得原来的密码
      init(){
        this.axios.post(this.HOST + '/teacehr/changePasswordByT',{
          accountNumber: sessionStorage.getItem('teacherName')
        }).then(result => {
          // console.log(result.data);
